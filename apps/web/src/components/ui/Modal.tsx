...
export default function Modal({ title, open, onClose, children, footer, size = 'default' }: ModalProps) {
  ...
  const modalContent = (
    ...
    >
      <div className={`bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-2xl shadow-2xl w-full ${sizeClasses[size]} max-h-[90vh] flex flex-col my-auto border border-slate-200/80 dark:border-slate-800 overflow-hidden`}>
        {/* Modal Header */}
