...
export default function Navbar({ user, onMenuToggle }: NavbarProps) {
  ...
  const roleColor: Record<string, string> = {
    Student: 'text-amber-800 bg-amber-50 border-amber-200/80',
    Coordinator: 'text-emerald-800 bg-emerald-50 border-emerald-200/80',
    Supervisor: 'text-purple-800 bg-purple-50 border-purple-200/80',
    ProgramHead: 'text-sky-800 bg-sky-50 border-sky-200/80',
    Admin: 'text-rose-800 bg-rose-50 border-rose-200/80',
  };
  const roleBadgeStyle = roleColor[user.role] || 'text-slate-700 bg-slate-100 border-slate-200';

  return (
    <header className="sticky top-0 z-30 w-full bg-white/90 backdrop-blur-md border-b border-slate-200/80 h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-all select-none">
      ...
      {/* Right: Security Pill, Notifications, and Top-Right Profile Dropdown */}
      <div className="flex items-center gap-2.5 sm:gap-4 shrink-0">
        ...
        <div className="hidden md:flex items-center gap-2 px-2.5 py-1 rounded-full bg-slate-50 border border-slate-200/80 text-[11px] text-slate-600">
          ...
          <span className="font-semibold text-slate-700">CdM Cloud</span>
          <span className="text-slate-300">•</span>
          <span className="text-slate-400 font-mono text-[10px]">ISO/IEC 25010</span>
        </div>
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            aria-haspopup="true"
            className="flex items-center gap-2.5 p-1 sm:pr-2.5 rounded-full hover:bg-slate-100 transition-all cursor-pointer group focus:outline-none focus:ring-2 focus:ring-[#0A3D24]/20"
          >
            ...
            <div className="hidden sm:flex flex-col text-left">
              <span className="text-xs font-bold text-slate-900 group-hover:text-[#0A3D24] transition-colors truncate max-w-[130px] leading-tight">
                {user.full_name}
              </span>
            ...
            <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-150 hidden sm:block ${dropdownOpen ? 'rotate-180 text-slate-700' : ''}`} />
          </button>

          {/* Floating Dropdown Menu */}
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white border border-slate-200/90 shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150 select-none">
              ...
              <div className="py-1">
                <Link
                  ...
                  className="flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium text-slate-700 hover:text-slate-900 hover:bg-slate-50 transition-colors"
                >
