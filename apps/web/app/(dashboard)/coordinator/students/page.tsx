            ...
            <tbody className="divide-y divide-slate-50">
              {students.map(student => (
                <tr key={student.user_id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-slate-700">{student.account_status}</td>
                  <td className="px-5 py-4 text-slate-700">{new Date(student.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
